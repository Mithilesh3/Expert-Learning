  ...
  return (
    <div className="space-y-4">
      ...
      <form onSubmit={async (e) => {
        ...
        <button type="submit" className="rounded-md bg-[#4F46E5] px-4 py-2 text-sm font-semibold text-white sm:col-span-3">Add Resource</button>
      </form>
      <div className="space-y-2 rounded-2xl border border-white/10 bg-[rgba(15,23,42,0.8)] p-5">
        {resources.map((resource) => (
          <article key={resource.id} className="rounded-lg border border-white/10 bg-white/5 p-3">
            ...
            <a href={resource.url} target="_blank" rel="noreferrer" className="mt-1 inline-flex text-xs text-[#4F46E5]">{resource.url}</a>
            <div className="mt-2 flex gap-3 text-xs">
              <button type="button" onClick={async () => { await updateResource(resource.id, { locked: !resource.locked }); await load(); }} className="text-[#4F46E5]">{resource.locked ? "Unlock" : "Lock"}</button>
